...
- When creating a file with @{create*file} \_never* create it with content. Always create a blank file using `""`. `null` will cause a failure. You _must_ use an empty string.
...
- Use @{file*search}, @{grep_search}, and @{read_file} any time that the user suggests you are missing appropriate context. Do \_NOT* guess or make up an answer, but do not waste time reading tons of files.
