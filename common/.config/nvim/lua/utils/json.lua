...
local function grok_number(self, text, start, options)
  ...
  if options.decodeNumbersAsObjects then
    ...
    if not options.decodeIntegerObjectificationLength and not options.decodeDecimalObjectificationLength then
      ...
      objectify = true
    elseif
      (options.decodeIntegerObjectificationLength and (integer_part:len() >= options.decodeIntegerObjectificationLength or exponent_part:len() > 0))
      or (options.decodeDecimalObjectificationLength and (decimal_part:len() >= options.decodeDecimalObjectificationLength or exponent_part:len() > 0))
    then
    ...
    --
    if
      (options.decodeIntegerStringificationLength and (integer_part:len() >= options.decodeIntegerStringificationLength or exponent_part:len() > 0))
      or (options.decodeDecimalStringificationLength and (decimal_part:len() >= options.decodeDecimalStringificationLength or exponent_part:len() > 0))
    then
  ...
  if #number_keys > 0 then
    ...
    for _, number_key in ipairs(number_keys) do
      ...
      if map[string_key] == nil then
        ...
        self:onEncodeError(
          'conflict converting table with mixed-type keys into a JSON object: key ' .. number_key .. ' exists both as a string and a number.',
          etc
        )
      end
    end
