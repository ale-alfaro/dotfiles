...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      _G.Utils.notify.error(lines, { title = 'Treesitter' })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      _G.Utils.notify.info 'Installing `tree-sitter-cli` with `mason.nvim`...'
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            _G.Utils.notify.info 'Installed `tree-sitter-cli` with `mason.nvim`.'
            cb(true)
          else
