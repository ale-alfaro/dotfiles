...
M.NOTE_TYPES = { fleeting = true, literature = true, permanent = true, reference = true }
M.REQUIRED_KEYS = { 'note_type', 'categories', 'created', 'last', 'tags' }
...
function M.is_note_path(rel)
  local top = rel:match('^([^/]+)/')
  return top and M.NOTE_TYPES[top] and rel:match('%.md$') ~= nil
end

---@param abs_path string
---@return string  Date as YYYY-MM-DD from file mtime.
function M.mtime_date(abs_path)
  local s = vim.uv.fs_stat(abs_path)
  return os.date('%Y-%m-%d', s and s.mtime.sec or os.time())
end
...
---@param abs_path string
---@param folder string
function M.set_note_type(abs_path, folder)
...
---@param abs_path string
---@param folder string
function M.autofill(abs_path, folder)
  local lines = readlines(abs_path)
  if not lines then
    return
  end
  local mt = M.mtime_date(abs_path)
  local defaults = {
...
---@param abs_path string
---@param tag string
function M.add_tag(abs_path, tag)
  local lines = readlines(abs_path)
  if not lines then
    return
  end
...
---@param abs_path string
---@param tag string
function M.remove_tag(abs_path, tag)
  local lines = readlines(abs_path)
  if not lines then
    return
  end
...
---@param old string
---@param new string
function M.rename_tag(abs_path, old, new)
  local lines = readlines(abs_path)
  if not lines then
    return
  end
...
---@param abs_path string
---@param cat string
function M.remove_category(abs_path, cat)
  local lines = readlines(abs_path)
  if not lines then
    return
  end
