...
function M.add_context_from_explorer_visual(buf_id)
  ...
  for i = from_line, to_line do
    ...
    if fs_entry.fs_type == nil and fs_entry.path == nil then
      ...
      vim.notify('Line ' .. entry .. ' does not have proper format. Did you modify without synchronization?',
        vim.log.levels.WARN)
    end
    if fs_entry.fs_type == nil and fs_entry.path ~= nil then
      local path_resolved = vim.fn.resolve(fs_entry.path)
      local symlink_info = path_resolved == fs_entry.path and '' or
          (' Looks like miscreated symlink (resolved to ' .. path_resolved .. ').')
...
function M.setup(opts)
  opts.memory = {
    ...
    parsers = { gemini = require 'custom.ai.memory.parsers.gemini' },
    opts = {
      chat = {
        enabled = true,
