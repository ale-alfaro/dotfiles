...
function M.run_python_selection()
  ...
  local get_visual_selection = function()
    ...
    return table.concat(vim.ensure_list(lines), '\n')
  end
      ...
      if data and #data > 1 then
        ...
        if output and output:match '%S' then
          vim.notify(output, vim.log.levels.INFO, {
            title = 'Python Output',
            timeout = M.execution_config.notification_timeout,
          })
        end
      end
    end,
...
function M.run_python_function()
  ...
  local run_function = function(func_name)
    ...
    vim.fn.jobstart(M.execution_config.run_command .. ' ' .. vim.fn.shellescape(temp_file), {
      on_stdout = function(_, data)
        if data and #data > 1 then
          local output = table.concat(data, '\n')
          if output and output:match '%S' then
...
function M.run_file()
  ...
  if current_file and current_file ~= '' then
    ...
    vim.fn.jobstart(M.execution_config.run_command .. ' ' .. vim.fn.shellescape(current_file), {
      on_stdout = function(_, data)
        if data and #data > 1 then
          local output = table.concat(data, '\n')
          if output and output:match '%S' then
