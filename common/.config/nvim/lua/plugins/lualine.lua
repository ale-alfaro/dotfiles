...
local function get_context()
  ...
  if vim.bo.filetype == 'startify' and result ~= '' then
    result = result .. ': v' .. tostring(vim.version().major) .. '.' .. tostring(vim.version().minor) .. '.' .. tostring(vim.version().patch)
  end
  return result
end
            ...
            function()
              if
                ...
                and vim.iter(vim.lsp.get_clients { bufnr = 0 }):any(
                  ...
                  function(cli)
                    return cli:supports_method(vim.lsp.protocol.Methods.textDocument_formatting, 0)
        ...
        lualine_z = {
          {
            ...
            cond = function()
              return vim.b.ai_raw_response ~= nil or (_G.codecompanion_chat_metadata and _G.codecompanion_chat_metadata[vim.api.nvim_get_current_buf()] ~= nil)
            end,
          },
        },
      },
        ...
        lualine_y = {
          {
            ...
            cond = function()
              return vim.list_contains({ 'python' }, vim.bo.filetype)
            end,
          },
        },
