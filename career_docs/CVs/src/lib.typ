...
#let __format_contact_items(author, item-inset: (:)) = {
    ...
    if "phone" in author {
        let phone-digits = author.phone.replace(
            regex("[^0-9+]"),
            "",
        )
        items.push(
            contact-item(
                (text: author.phone, icon: phone-icon, link: phone-digits),
                link-prefix: "tel:",
            ),
        )
    }
...
#let bold-secondary-justified-header(primary, secondary) = {
...
#let resume(
    ...
    let positions-smallcaps = if positions-smallcaps == none {
        use-smallcaps
    } else {
        positions-smallcaps
    }
        ...
        set document(
            ...
            title: (
                author.firstname
                    ...
                    + " - "
                    + lflib
                        ._linguify("resume", lang: language, from: lang_data)
                        .ok
            ),
            description: desc,
            keywords: keywords,
    ...
    // Contact section (address, if present, is folded in as the first item)
    let contacts = {
        set box(height: 9pt)
        set text(size: 9pt, weight: "regular", style: "normal")

        let address-item = if "address" in author {
        ...
        let items = __format_contact_items(
            author,
            item-inset: contact-items-inset,
        )
        let all-items = if address-item != none {
            (address-item,) + items
        } else {
            items
        }
        align(center, all-items.join(contact-items-separator))
    }
